...
function recalc_layout(state) {
  const is_portrait =
    state.config.rotation == 90 || state.config.rotation == 270;
  let width, height;
  if (is_portrait) {
...
const ChildTile = (function () {
  ...
  function config_value(key, default_value, convert) {
    return {
      ...
      set(value) {
        if (convert != undefined) value = convert(value);
...
function load_childs(assets) {
  return new Promise(function (resolve, reject) {
    ...
    for (var name in assets) {
      ...
      if (asset.filetype != "child") continue;
...
const store = new Vuex.Store({
  ...
  mutations: {
    init(state, { assets, node_assets, config }) {
      ...
      for (let i = 0; i < schedules.length; i++) {
        state.config.schedules[i].ends = null;
      }
      console.log(state.config);
      recalc_layout(state);
    },
    ...
    schedule_set_hour(state, { schedule_id, hour, on }) {
      ...
      while (hours.length < 24 * 7) hours.push(true);
    ...
    schedule_set_interval(state, { schedule_id, interval }) {
      Vue.set(
        state.config.schedules[schedule_id].scheduling,
        "interval",
        interval
      );
    },
    schedule_set_span(state, { schedule_id, span_id, span }) {
      Vue.set(
        state.config.schedules[schedule_id].scheduling.spans,
        span_id,
        span
      );
    },
    ...
    page_delete(state, { schedule_id, page_id }) {
      if (
        state.config.scratch.debug_schedule_id == schedule_id &&
    ...
    page_tile_create(state, { schedule_id, page_id, tile }) {
      ...
      Vue.set(
        state.config.schedules[schedule_id].pages[page_id],
        "selected",
        page.tiles.length - 1
      );
    },
    ...
    page_tile_set_pos(state, { schedule_id, page_id, tile_id, pos }) {
      var tile =
        state.config.schedules[schedule_id].pages[page_id].tiles[tile_id];
    ...
    layout_tile_set_config(state, { layout_id, tile_id, key, value }) {
      Vue.set(
        state.config.layouts[layout_id].tiles[tile_id].config,
        key,
        value
      );
    },
  ...
  actions: {
    ...
    page_tile_set_asset(
      { commit },
      { schedule_id, page_id, tile_id, asset_spec }
    ) {
      commit("page_tile_set_asset", {
        schedule_id,
        page_id,
        tile_id,
        asset_spec,
...
Vue.component("config-ui", {
  ...
  computed: {
    ...
    background() {
      ...
      if (background == undefined) background = [0, 0, 0, 0];
      return background;
    },
    background_a: {
      get() {
...
Vue.component("schedule-panel", {
  ...
  computed: {
    ...
    schedule_id() {
      return this.$store.state.schedule_id;
    },
    schedule() {
      return this.config.schedules[this.schedule_id];
    },
    pages: {
      get() {
    ...
    scheduling() {
      return this.schedule.scheduling;
    },
    duration() {
      var duration = 0;
...
Vue.component("page-compact", {
  ...
  computed: {
    ...
    page() {
      return this.schedule.pages[this.page_id];
    },
    duration() {
      return this.page.duration;
    },
    auto_duration() {
      return this.page.auto_duration;
    },
    is_disabled() {
    ...
    is_debugged() {
      return (
        this.config.scratch.debug_schedule_id == this.schedule_id &&
...
Vue.component("tile-editor", {
  ...
  computed: {
    ...
    selected_tile() {
      return this.selected_tile_id != undefined
        ? this.tiles[this.selected_tile_id]
        : null;
    },
    style() {
      return {
...
Vue.component(
  ...
  TileOption.extend({
    ...
    computed: {
      transparent_color: {
        get() {
          return this.config.transparent_color || "#ffffff";
        },
        set(value) {
          this.onSetValue("transparent_color", value);
        },
      },
    },
  })
...
Vue.component(
  ...
  TileOption.extend({
    ...
    computed: {
      mode() {
        return this.config.mode || "digital_clock";
      },
      timezone() {
        return this.config.timezone || null;
      },
      type() {
        return this.config.type || "hms";
      },
      style() {
        return this.config.style || 1;
      },
      align() {
        return this.config.align || "center";
      },
      movement() {
        return this.config.movement || "dynamic";
      },
      color: {
        get() {
          return this.config.color || "#333333";
        },
        set(value) {
          this.onSetValue("color", value);
        },
      },
    },
  })
...
Vue.component(
  ...
  TileOption.extend({
    ...
    computed: {
      color: {
        get() {
          return this.config.color || "#ffffff";
        },
        set(value) {
          this.onSetValue("color", value);
        },
      ...
      alpha: {
        get() {
          return this.config.alpha || 1;
        },
        set(value) {
          this.onSetValue("alpha", parseFloat(value));
        },
      },
    },
  })
...
Vue.component("tile-box", {
  ...
  computed: {
    ...
    x1() {
      return this.tile.x1 || 0;
    },
    y1() {
      return this.tile.y1 || 0;
    },
    x2() {
      return this.tile.x2 || this.screen.width;
    },
    y2() {
      return this.tile.y2 || this.screen.height;
    },
    style() {
      return {
...
Vue.component("tile-detail", {
  ...
  computed: {
    ...
    width: {
      get() {
        return this.tile.x2 - this.tile.x1;
      },
      set(value) {
        value = this.toNumber(value);
        this.$emit("onPositionTile", {
    ...
    height: {
      get() {
        return this.tile.y2 - this.tile.y1;
      },
      set(value) {
        value = this.toNumber(value);
        this.$emit("onPositionTile", {
...
Vue.component("date-select", {
  ...
  methods: {
    ...
    onSelect(value) {
      if (value != "") this.$emit("onSetDate", value);
    },
    onChange(date) {
...
Vue.component("time-editor", {
  ...
  computed: {
    date_note() {
      ...
      var now = moment();
      if (starts) starts = moment(starts);
      if (ends) ends = moment(ends);
    ...
    onPreset(name) {
      if (name == "never") {
        ...
        for (var i = 0; i < 24 * 8; i++) {
          ...
          if (hours[i]) this.$emit("onHourChange", i, daytime);
        }
      } else if (name == "invert") {
        var hours = this.scheduling.hours;
        for (var i = 0; i < 24 * 8; i++) {
...
Vue.component("layout-editor", {
  ...
  computed: {
    ...
    layout() {
      return this.layouts[this.layout_id];
    },
  },
  methods: {
    onAddTile(tile) {
